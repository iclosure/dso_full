 ...
 *                    FFT & IFFT
 */
void MATH_FFT (int x[], I8 m, I8 flag);  // 计算实序列的基二快速傅里叶变换.
void MATH_IFFT(int x[], I8 m);           // 计算共轭对称实序列的基二快速傅里叶反变换，其变换结果是实数.
 ...
 *                    插值函数
 */
void  MATH_SincInterp(I16 sample[],I16 SampleN, I16 Interp[],I16 InterpN);  // 正弦内插函数
 ...
 */
// 可变类型的比较函数
int MATH_compareUp(const void *_key , const void *_base);    // 升序排序
int MATH_compareDown(const void *_key , const void *_base);  // 降序排序
