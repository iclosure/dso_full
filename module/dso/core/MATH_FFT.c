...
void MATH_FFT(int x[], I8 m, I8 flag)
{
...
//////////////////////////////////////
//////////////////////////////////////
void MATH_rFFT(float x[], I16 m, I8 flag)
{
...
void MATH_FFT_test(void)
{
    ...
    //MATH_rFFT(fData,SampleM,1);
	for(i=0; i<SampleN>>1; i++)
		data[i] = - fData[i];
	GUI_SetColor(GUI_BLACK);
	GUI_DrawGraph(data,SampleN>>1,0,250);
	
	MATH_FFT(data1,SampleM,0);
	MATH_IFFT(data1,SampleM);
	MATH_FFT(data1,SampleM,1);
	for(i=0; i<SampleN>>1; i++)
	...
	GUI_DispFloatMin(fData[2]/100, 4);
	GUI_DispString("mV");
	GUI_DispStringAt("     Freq: ",50,116);
	GUI_DispFloatMin(fData[3], 4);
	GUI_DispString("Hz");
}
