...
dd_newReader(int (*read)(ddReader* r, void* buf, int bytes),
			 ...
			 void* data, int foo)
{
	ddReader* r = (ddReader *)(dd_malloc(sizeof(struct _ddReader)));
...
bufferRead(ddReader* r, void* buf, int bytes)
{
	...
	memcpy(buf, (void *)((unsigned char *)(r->data) + r->fileOffset), bytes);
	r->fileOffset += bytes;
...
{
	r->dictionaryList = (struct _dictionary *)(dd_realloc(r->dictionaryList, (r->nDictionaries + 1) * sizeof(struct _dictionary)));
...
readUInt32(ddReader* r)
{
	...
	data.a[1] = readUInt8(r) ; 
	data.a[2] = readUInt8(r) ; 
	data.a[3] = readUInt8(r) ; 
	return (unsigned long)(data.val) ; 
...
{
	unsigned char* buf = (unsigned char *)(dd_malloc(length));
