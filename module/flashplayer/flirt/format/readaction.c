...
addOpcode(struct opcodeList* list)
{
	...
	ddActionOpcodeInit(&code);
	list->opcodes = (struct opcodeWithOffset*)(dd_realloc(list->opcodes, (list->opcount+1) * sizeof(struct opcodeWithOffset)));
...
{
	int i;
	int* offsets = (int *)(dd_malloc(list->opcount * sizeof(int)));
	ddAction* newaction;
...
readDeclareNames(ddReader* r, struct opcodeList* list)
{
	...
	list->dictionary = (char **)(dd_malloc(dictionarySize * sizeof(char*)));
...
readDefineFunction(ddReader* r, struct opcodeList* list, int offset)
{
	...
	args = (char **)(dd_malloc(sizeof(char*) * nargs));
...
readActionOp(ddReader* r, struct opcodeList* list)
{
	...
	ddActionCode code = (ddActionCode)(*ptr);
	unsigned int len = 0;
