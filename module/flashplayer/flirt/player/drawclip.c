...
{
	ddDrawClip* c = (ddDrawClip *)(dd_malloc(sizeof(ddDrawClip)));
	ddDrawClip_init(c);
...
ddDrawClip_addDisplayItem(ddPlayer* player, ddDrawClip* clip, ddCharacter* c, int level, const char* name, int maskLevel)
{
	...
	if ( d != NULL )
	{
		...
		if ( c->type == MOVIECLIP_CHAR )
		{
			...
			if ( name == NULL )
			{
				clipname = (char *)(dd_malloc(16)); /* "instancexxxxx" */
				sprintf(clipname, "inst%i", level);
			}
			else
...
ddDrawClip_getDisplayList(ddDrawClip* clip, ddDrawable*** outItems, int* outNItems)
{
	...
	ddDrawable** list_pp = NULL;
	int count = 0;
	
	while ( d != NULL )
	{
		list_pp = (ddDrawable **)(dd_realloc(list_pp, (count + 1) * sizeof(ddDrawable*)));
		list_pp[count] = d;
		++count;
		d = d->next;
	}
