...
dd_newText()
{
	ddText* text = (ddText *)(dd_malloc(sizeof(ddText)));
	struct _ddTextRecord * record = (struct _ddTextRecord *)(dd_malloc(sizeof(struct _ddTextRecord)));
...
ddText_addGlyph(ddText* text, int glyph, fixed advance)
{
	...
	record->glyphs = (int *)(dd_realloc(record->glyphs, (nGlyphs + 1) * sizeof(int)));
	record->advances = (fixed *)(dd_realloc(record->advances, (nGlyphs + 1) * sizeof(fixed)));
