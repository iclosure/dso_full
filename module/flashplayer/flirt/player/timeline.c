...
dd_newTimeline()
{
	ddTimeline* t = (ddTimeline *)(dd_malloc(sizeof(ddTimeline)));
...
{
	int i;
	ddTimelineItem* newitem = (ddTimelineItem *)(dd_malloc(sizeof(ddTimelineItem)));
...
{
	int i;
	ddTimelineItem* newitem = (ddTimelineItem *)(dd_malloc(sizeof(struct _ddTimelineItem)));
...
ddTimeline_nextFrame(ddTimeline* timeline)
{
	...
	timeline->frames = (ddTimelineFrame *)(dd_realloc(timeline->frames, (nFrames + 1) * sizeof(ddTimelineFrame)));
...
ddTimeline_updateClipDisplay(ddPlayer* player, ddTimeline* timeline, ddDrawClip* clip)
{
	...
	while ( item != NULL )
	{
		...
		if ( drawable != NULL )
		{
			...
			if ( new_bool && item->character->type == MOVIECLIP_CHAR )
			{
...
ddTimeline_playFrameSounds(ddPlayer* player, ddTimeline* timeline, ddDrawClip* clip)
{
	...
	while ( sound != NULL )
	{
		...
		else
		{
			if ( sound->sound == NULL )
			{
				c = ddMovieClip_getCharacter(player->player, sound->characterid);
