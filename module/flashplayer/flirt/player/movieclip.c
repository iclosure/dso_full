...
{
	ddMovieClip* clip = (ddMovieClip *)(dd_malloc(sizeof(ddMovieClip)));
...
{
	char* name = (char *)(dd_malloc(16));

	ddDrawClip* d = dd_newDrawClip(player, clip);

	sprintf(name, "_level%i", level);
...
{
	clip->labels = (struct _ddMovieClipFrameLabel *)(dd_realloc(clip->labels,
...
void
ddMovieClip_addLibrarySymbol(ddMovieClip* player, int id_v, char* name)
{
	player->librarySymbols = (struct _ddMovieClipLibrarySymbol *)(dd_realloc(player->librarySymbols,
									 (player->nLibrarySymbols + 1) * sizeof(struct _ddMovieClipLibrarySymbol)));

	player->librarySymbols[player->nLibrarySymbols].charID = id_v;
...
ddMovieClip_registerClass(ddMovieClip* player, char* name, ddActionFunction* ctor)
{
	...
	{
		if ( stricmp(name, player->librarySymbols[i].name) == 0 )
		{
