...
static void
ddDrawable_getUpdateList(ddDrawable* d, ddUpdateList* list_u, ddMatrix matrix)
{
	d->getUpdateList(d, list_u, matrix);

	if ( d->flags & DRAWABLE_NEEDSUPDATE )
		ddUpdateList_includeRect(list_u, d->bounds);

	d->flags = 0;
}

static void
ddDrawable_drawInImage(ddDrawable* d, ddImage* image, ddUpdateList* list_u,
					   ddMatrix matrix, ddCXform cXform, ddRect clipRect)
{
	d->drawInImage(d, image, list_u, matrix, cXform, clipRect);
}
...
void
ddDrawable_replace(ddDrawable* old, ddDrawable* new_v);
...
static int ddDrawable_getDepth(ddDrawable* d)
{
	return d->depth;
}

static void ddDrawable_setNeedsUpdate(ddDrawable* d)
{
	d->flags |= DRAWABLE_NEEDSUPDATE;
}

static void ddDrawable_setMark(ddDrawable* d)
{
	d->flags |= DRAWABLE_MARK;
}
static void ddDrawable_clearMark(ddDrawable* d)
{
	d->flags &= ~DRAWABLE_MARK;
}
static int ddDrawable_getMark(ddDrawable* d)
{
