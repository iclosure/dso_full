...
static void
ddDrawShape_getUpdateList(ddDrawable* d, ddUpdateList* list_p, ddMatrix matrix)
{
	...
	ddShapeInstance_getUpdateList(s->shape, list_p, m);
	d->bounds = s->shape->bounds;
}


static void
ddDrawShape_drawInImage(ddDrawable* d, ddImage* image, ddUpdateList* list_p,
...
{
	ddDrawShape* s = (ddDrawShape *)(dd_malloc(sizeof(ddDrawShape)));
