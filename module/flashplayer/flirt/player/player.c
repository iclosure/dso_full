...
{
	ddPlayer* player = (ddPlayer *)(dd_malloc(sizeof(ddPlayer)));
...
ddPlayer_executeFrameActions(ddPlayer* player, ddStepMode mode)
{
	...
	while ( d != NULL )
	{
		...
		{
			ddAction* action = (ddAction *)(d->item);
			ddActionObject* object = (ddActionObject *)(d->next->item);
...
{
	ddListEntry** list_pp = &dlist->head;
	ddListEntry** endPtr = &dlist->end;

#ifdef DD_LOG
	ddListEntry* entry = *list_pp;
	...
	entry = (ddListEntry *)(dd_malloc(sizeof(ddListEntry)));
#else
...
{
	ddListEntry* newentry = (ddListEntry *)(dd_malloc(sizeof(ddListEntry)));
