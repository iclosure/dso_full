...
void
ddEventHandler_addHandler(ddEventHandler** list_pp,
						  ddDrawClip* drawclip, ddAction* action)
{
	ddEventHandler* newhandler = (ddEventHandler *)(dd_malloc(sizeof(struct _ddEventHandler)));

	newhandler->clip = drawclip->actionClip;
	newhandler->action = action;
	newhandler->next = *list_pp;
	*list_pp = newhandler;
}


void
ddEventHandler_clear(ddEventHandler* list_p)
{
	while ( list_p != NULL )
	{
		ddEventHandler* next = list_p->next;
		dd_free(list_p);
		list_p = next;
	}
}
...
void
ddEventHandler_removeHandler(ddEventHandler** list_pp, ddDrawClip* drawclip)
{
	ddActionMovieClip* clip = drawclip->actionClip;
	ddEventHandler* handler = *list_pp;
