...
dd_newAction()
{
	ddAction* action = (ddAction *)(dd_malloc(sizeof(struct _ddAction)));
	action->next = NULL;
	action->opcodes = NULL;
	action->length = 0;
	return action;
...
{
	ddActionClass* class_p = dd_newActionClass();
	ddActionFunction* ctor = dd_newActionNativeFunction(constructor, nargs);

	ddActionFunction_setPrototype(ctor, (ddActionObject*)class_p);
...
ddAction_getBreakpoints(ddAction* action, int** outLines, int* outNBreaks)
{
	...
	for ( i = 0; i < action->length; ++i )
	{
		...
		list_p = (int *)(dd_realloc(list_p, (count + 1) * sizeof(int)));
		list_p[count] = i;
		++count;
	}
