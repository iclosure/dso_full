...
ddAction_newOpcode(ddAction* action)
{
	...
	action->opcodes = (ddActionOpcode *)(dd_realloc(action->opcodes, (action->length + 1) * sizeof(ddActionOpcode)));
...
void ddActionOpcodeInit(struct _ddActionOpcode *_pddActionOpcode)
{
	_pddActionOpcode->arg=NULL;
	_pddActionOpcode->op=ACTION_END;
	_pddActionOpcode->data=0;
	_pddActionOpcode->val.type=VALUE_NULL;
//	_pddActionOpcode->val.data=0;
}
void
...
action_stringConcat(ddActionContext* context)
{
	...
	str1 = (char *)(dd_realloc(str1, strlen(str1) + strlen(str2) + 1));
	strcat(str1, str2);
...
action_getProperty(ddActionContext* context)
{
	...
	if ( targetObj != NULL && targetObj->proto == (ddActionObject*)ddActionMovieClipClass )
	{
		push(ddActionMovieClip_getPropertyNum(context->player,
										...
										(ddClipProperty)(ddActionValue_getIntValue(val))));
	}
	else
		push(ddNullValue);
...
action_setProperty(ddActionContext* context)
{
	...
	if ( targetObj != NULL && targetObj->proto == (ddActionObject*)ddActionMovieClipClass )
	{
		ddActionMovieClip_setPropertyNum((ddActionMovieClip*)targetObj,
								   (ddClipProperty)(ddActionValue_getIntValue(prop)),
								   (value));
	}
...
action_newAdd(ddActionContext* context)
{
	...
	else if ( a.type == VALUE_STRING || b.type == VALUE_STRING )
	{
		...
		bstr = (char *)(dd_realloc(bstr, strlen(bstr) + strlen(astr) + 1));
		strcat(bstr, astr);
		dd_free(astr);
...
action_add_string(ddActionContext* context, const char* str)
{
	...
	lstr = (char *)(dd_realloc(lstr, strlen(lstr) + strlen(str) + 1));
	strcat(lstr, str);
...
static void
action_namedNew(ddActionContext** contextPtr, const char* class_v)
{
	ddActionContext* context = *contextPtr;
	ddActionValue nargsval = pop();
	int nargs = ddActionValue_getIntValue(nargsval);

	ddActionValue ctorval = ddActionObject_getProperty(context, context->object, class_v);
	ddActionFunction* ctor;

	if ( ctorval.type == VALUE_UNDEF )
		ctorval = ddActionObject_getProperty(context, context->player->globals, class_v);
...
action_callNamedMethod(ddPlayer* player, ddActionContext** contextptr, const char* name)
{
	...
	if ( objectVal.type == VALUE_SUPER )
	{
		...
		if ( context->class_p != NULL )
			super = context->class_p;
...
action_callNamedFunctionNargs(ddPlayer* player, ddActionContext** contextptr, const char* name, int nargs)
{
	...
	{
		ddActionObject* class_p = context->object->proto;
		
		if ( context->class_p != NULL )
			class_p = context->class_p;

		if ( class_p->proto != NULL )
		{
			funcval = ddActionHash_findValue(class_p->proto->properties, "constructor");
			function = ddActionValue_getFunctionValue(funcval);
			
			ddActionFunction_call(function, context->object, contextptr, class_p->proto, nargs, DD_TRUE);
		}
		else
			push(ddNullValue);
	}
...
dd_doBytecodeAction(ddPlayer* player, ddActionContext** contextptr, ddActionOpcode* op)
{
	...
	switch ( op->op )
	{
		...
		case ACTION_GETTIMER:
			push(dd_newActionValue_double(dd_getTicks() - player->startTime));
			break;
