...
{
	ddActionString* strobj = (ddActionString *)(dd_malloc(sizeof(struct _ddActionString)));
...
String_charAt(ddActionObject* object, ddActionContext* context, int nargs)
{
	...
	charat = (char *)(dd_malloc(2*sizeof(char)));
	charat[0] = str[pos];
	charat[1] = '\0';
...
String_concat(ddActionObject* object, ddActionContext* context, int nargs)
{
	...
	while ( --nargs >= 0 )
	{
		...
		str = (char *)(dd_realloc(str, len + catlen + 1));
...
String_fromCharCode(ddActionObject* object, ddActionContext* context, int nargs)
{
	...
	char* charat = (char *)(dd_malloc(2 * sizeof(char)));
...
{
	char* sub = (char *)(dd_malloc(sizeof(char) * (to - from + 1)));
