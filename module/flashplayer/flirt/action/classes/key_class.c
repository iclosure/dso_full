...
Key_addListener(ddActionObject* object, ddActionContext* context, int nargs)
{
	...
	if ( val.type == VALUE_OBJECT )
	{
		...
		key->listeners = (ddActionObject **)(dd_realloc(key->listeners, (key->nListeners + 1) * sizeof(ddActionObject*)));
...
dd_initKeyObject()
{
	ddActionKeyObject* key = (ddActionKeyObject *)(dd_malloc(sizeof(ddActionKeyObject)));
