...
dd_newActionObject()
{
	ddActionObject* object = (ddActionObject *)(dd_malloc(sizeof(struct _ddActionObject)));
...
{
	/* clean up properties */
	ddActionClass* class_p = object->nativeClass;

	while ( class_p != NULL && class_p->destroy == NULL )
		class_p = (ddActionClass*)(class_p->parent.proto);
	
	if ( class_p != NULL )
		class_p->destroy(object);
...
void
ddActionObject_setClass(ddActionObject* object, ddActionClass* class_p)
{
	object->nativeClass = class_p;
	
	ddActionObject_setPrototype(object, (ddActionObject*)class_p);
}
...
ddActionObject_getProperties(ddActionObject* object, char*** outNames, ddActionValue** outValues, int* outCount)
{
	...
	ddActionValue* values = (ddActionValue *)(dd_malloc(sizeof(ddActionValue) * count));
	char** names = (char **)(dd_malloc(sizeof(char*) * count));
	int i;
	int nval = 0;
