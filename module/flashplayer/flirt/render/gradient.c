...
dd_newGradient()
{
	ddGradient* g = (ddGradient *)(dd_malloc(sizeof(ddGradient)));
	g->entries = NULL;
	g->nGrads = 0;
	g->band = 0;
	return g;
...
{
	ddGradient* newg = (ddGradient *)(dd_malloc(sizeof(ddGradient)));
	newg->nGrads = g->nGrads;
	newg->entries = (struct _ddGradEntry *)(dd_malloc(newg->nGrads * sizeof(struct _ddGradEntry)));
...
ddGradient_add(ddGradient* g, ddColor color, UInt8 ratio)
{
	...
	g->entries = (struct _ddGradEntry *)(dd_realloc(g->entries, sizeof(struct _ddGradEntry) * (g->nGrads + 1)));
