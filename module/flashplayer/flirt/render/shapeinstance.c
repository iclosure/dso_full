...
{
	int i;
	ddShapeInstance* s = (ddShapeInstance *)(dd_malloc(sizeof(ddShapeInstance)));
	...
	s->fills = (ddFillInstance *)(dd_malloc(shape->nFills * sizeof(struct _ddFillInstance)));
...
ddShapeInstance_setGlyphColor(ddShapeInstance* s, ddColor color)
{
	...
	if ( s->nFills == 0 )
	{
		s->fills = (ddFillInstance *)(dd_realloc(s->fills, sizeof(struct _ddFillInstance)));
		s->nFills = 1;
	}
	else
