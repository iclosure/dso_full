...
dd_newShape()
{
	ddShape* s = (ddShape *)(dd_malloc(sizeof(struct _ddShape)));
...
{
	int i;
	ddShape* newshape = (ddShape *)(dd_malloc(sizeof(struct _ddShape)));

	newshape->nFills = shape->nFills;
	newshape->fills = (ddFill *)(dd_malloc(newshape->nFills * sizeof(ddFill)));

	for ( i = 0; i < newshape->nFills; ++i )
		newshape->fills[i] = ddFill_copy(shape->fills[i]);

	newshape->nRecords = shape->nRecords;
	newshape->records = (ddShapeRecord *)(dd_malloc(newshape->nRecords * sizeof(struct _ddShapeRecord)));
...
{
	s->records = (ddShapeRecord *)(dd_realloc(s->records,
...
{
	s->fills = (ddFill *)(dd_realloc(s->fills, sizeof(ddFill) * (s->nFills + 1)));
