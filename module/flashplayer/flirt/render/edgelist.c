...
static void 
ddEdgeList_order(ddEdgeList* list_p, int nVerts)
{
	int i;
	
	if ( list_p->sortedEdges == NULL )
		list_p->sortedEdges = (ddEdge **)(dd_malloc(sizeof(ddEdge*) * list_p->nEdges));
	
	for ( i = 0; i < list_p->nEdges; ++i )
		list_p->sortedEdges[i] = &list_p->edges[i];
	
	qsort(list_p->sortedEdges, nVerts, sizeof(ddEdge*), _Edge_compare);
}
				   ...
				   fixed startx, fixed starty, fixed endx, fixed endy)
{
	...
	b->list_p = (ddEdge *)(dd_realloc(b->list_p, sizeof(struct _ddEdge) * (b->nEdges + 1)));

	edge = &(b->list_p[b->nEdges]);
...
void
ddEdgeList_closePath(ddEdgeList* list_p, struct builderState *b)
{
	int i;
	
	list_p->bounds = b->bounds;
	list_p->nEdges = b->nEdges;
	list_p->edges = b->list_p;
	
	ddEdgeList_order(list_p, list_p->nEdges);
	
	// first edge determines if left/right fill is swapped
	
	for ( i = 0; i < list_p->nEdges; ++i )
	{
		ddEdge* edge = list_p->sortedEdges[i];
		...
		{
			list_p->swapFills = DD_TRUE;
			break;
		}

		if ( edge->rightfill != NULL )
		{
			list_p->swapFills = DD_FALSE;
			break;
		}
	}
...
{
	ddEdgeList* list_p = (ddEdgeList *)(dd_malloc(sizeof(ddEdgeList)));
	
	list_p->next = NULL;
	list_p->bounds = ddInvalidRect;
	list_p->edges = NULL;
	list_p->sortedEdges = NULL;
	list_p->nEdges = 0;
	list_p->index = 0;
	list_p->start = NULL;
	list_p->swapFills = DD_FALSE;
...
dd_newEdgeList(ddShapeInstance* s, ddMatrix matrix)
{
	...
	for ( i = 0; ; ++i )
	{
		...
		switch ( type )
		{
			...
			{
				ddEdgeList_closePath(list_p, &b);
				
				bounds = ddRect_containRect(bounds, list_p->bounds);
				
				list_p->next = dd_newEdgeList_empty();
				list_p = list_p->next;
	...
	s->bounds = ddRect_containRect(bounds, list_p->bounds);
...
void
ddEdgeList_recalcSlopes(ddEdgeList* list_p, ddMatrix matrix, ddRect* bounds)
{
	/*
	...
	*/
	...
	if ( list_p->next != NULL )
		ddEdgeList_recalcSlopes(list_p->next, matrix, bounds);
}


void
ddEdgeList_recalcCoords(ddEdgeList* list_p, ddMatrix matrix, ddRect* bounds)
{
	int i;
	...
	*bounds = ddInvalidRect;
	...
	if ( list_p->next != NULL )
		ddEdgeList_recalcCoords(list_p->next, matrix, bounds);
}


void
ddEdgeList_displaceEach(ddEdgeList* list_p, fixed dx, fixed dy)
{
	int i;
	
	for ( i = 0; i < list_p->nEdges; ++i )
	{
		list_p->edges[i].startx += dx;
		list_p->edges[i].starty += dy;
		list_p->edges[i].endx += dx;
		list_p->edges[i].endy += dy;
		
		ddEdge_setInitXY(&list_p->edges[i]);
	}
	
	list_p->bounds = ddRect_displace(list_p->bounds, dx, dy);

	if ( list_p->next != NULL )
		ddEdgeList_displaceEach(list_p->next, dx, dy);
}

void
ddEdgeList_displace(ddEdgeList* list_p, fixed dx, fixed dy, ddRect* bounds)
{
	ddEdgeList_displaceEach(list_p, dx, dy);
...
void
ddEdgeList_reset(ddEdgeList* list_p)
{
	ddEdge_unlinkList(list_p->start);
	
	list_p->start = NULL;
	list_p->index = 0;
	
	if ( list_p->next != NULL )
		ddEdgeList_reset(list_p->next);
}


void
ddEdgeList_advanceToScanline(ddEdgeList* list_p, int y, fixed clipLeft, fixed clipRight)
{
	// add all vertices starting on or before scanline y

	int idx = list_p->index;

	ddEdge* head = list_p->start;
	ddEdge* edge = head;
	...
	while ( idx < list_p->nEdges )
	{
		ddEdge* listedge = list_p->sortedEdges[idx];
...
void
ddEdgeList_removeEdgesBeforeScanline(ddEdgeList* list_p, int y, fixed clipLeft, fixed clipRight)
{
	// remove all vertices ending before scanline y or tracked outside of clip range

	ddEdge* edge = list_p->start;
	ddEdge* head = edge;
	...
	if ( list_p->next != NULL )
		ddEdgeList_removeEdgesBeforeScanline(list_p->next, y, clipLeft, clipRight);	
}
...
void
doFill(ddEdgeList* list_p, ddEdge* leftedge, ddEdge* rightedge, int endleft, int endright)
{
	// fill in solid area
...
void
ddEdge_render(ddEdge* edge, ddEdgeList* list_p, int endleft, int endright, int y)
{
